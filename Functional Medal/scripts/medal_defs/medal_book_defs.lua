...
local function doSacrifice(inst,player,itemlist,summonedlist,noconsume)
	...
	if #ents>0 then
		for i,v in ipairs(ents) do
			...
			if itemlist[v.prefab] then
				--需要通过特殊检查才能进行献祭
				if itemlist[v.prefab].testfn==nil or itemlist[v.prefab].testfn(player) then
					--需要有特定标签才能献祭
					if not itemlist[v.prefab].notag or not v:HasTag(itemlist[v.prefab].notag) then
						...
						if consumenum>0 and not noconsume then
							...
							if not itemlist[v.prefab].nofx then
								SpawnMedalFX(itemlist[v.prefab].fx or "spawn_fx_tiny",v)
							end
							--移除献祭品
	...
	chance_list = disorganize(chance_list,GetMedalDestiny(inst))--打乱
	return GetMedalRandomItem(chance_list,inst)--返回权重增值后的随机key
end
...
local chesspiece_list={
	land={
		...
		medal_spacetime_snacks={chance=50,key="medal_spacetime_devourer",fx="pocketwatch_heal_fx",testfn=function(inst)
			return TheWorld and TheWorld:HasTag("forest") and not TheSim:FindFirstEntityWithTag("medal_spacetime_devourer")
		end},--时空碎片--时空吞噬者
		voidcloth={chance=6,key="medal_shadowthrall_screamer",fx="collapse_small",testfn=function(inst)
...
local unsolved_item_list={
	land={
		atrium_key={chance=15*un_mult,key="yuangu",testfn=function(inst)
			return not (TheWorld and TheWorld:HasTag("cave"))--不能在洞穴献祭
		end},--远古钥匙--远古祭坛
		minotaurhorn={chance=15*un_mult,key="yuangu",testfn=function(inst)
		...
		-- bottled_moonlight={chance=6*un_mult,key="fullmoon",testfn=function(inst)
		-- 	return not (TheWorld and TheWorld:HasTag("cave"))--不能在洞穴献祭
		-- end},--瓶装月光--月圆
...
local plant_item_list={
	cave_banana={chance=1*un_mult,key="bananabush"},--香蕉--香蕉丛
	...
	},
	{
		...
		readfn=function(inst,reader)
			...
			if reader:GetCurrentPlatform() ~= nil or (reader.components.drownable~= nil and reader.components.drownable:IsOverWater()) then
				...
				if monster_code == "medal_spacetime_devourer" then--时空吞噬者生成方式比较特殊
				...
				elseif monster_code == "medal_shadowthrall_screamer" then--驱光遗骸有特殊的生成动画
					local fossil_stalker = FindEntity(inst, TUNING_MEDAL.BOOK_SACRIFICE_RADIUS, function(inst)
						return inst.prefab=="fossil_stalker" and inst.form == 2 and inst.moundsize == 8 
					end, {"structure","trader"}, nil, nil)
					if fossil_stalker ~= nil then
						...
						fossil_stalker:DoTaskInTime(2, function() 
							if fossil_stalker and fossil_stalker:IsValid() then
								monster = SpawnPrefab("medal_shadowthrall_screamer")
								if monster then
