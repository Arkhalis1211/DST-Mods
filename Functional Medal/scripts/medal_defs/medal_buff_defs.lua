...
-------------------------------------流血debuff-------------------------------------------------
local function OnTickBleed(inst,target)
	if target:IsValid() and target.components.health ~= nil and not target.components.health:IsDead() and not target:HasTag("playerghost") then
		target.components.health:DoDelta(TUNING_MEDAL.MEDAL_BUFF_BLEED_DAMAGE, nil, "buff_medal_bleed",nil,nil,true)--无视防御
	else
		 inst.components.debuff:Stop()
	end
end
...
--叠毒
local function DoBleed(inst, target)
	...
	if target ~= nil and target:IsValid() and target.components.health ~= nil and not target.components.health:IsDead() then
		inst.bleed_task = inst:DoPeriodicTask(1, OnTickBleed, nil, target)
	end
end
...
----------------------------------------红晶标记-------------------------------------------------
local NO_TAGS_NO_PLAYERS =	{ "bramble_resistant", "INLIMBO", "notarget", "noattack", "flight", "invisible", "wall", "player", "companion" , "noredattack" }
		...
		if marks >= 5 then
            ...
            for i, v in ipairs(TheSim:FindEntities(x, y, z, ATTACKRANGE, COMBAT_TARGET_TAGS, NO_TAGS_NO_PLAYERS)) do
                if v ~= target and v:IsValid() and v.components.health ~= nil and not v.components.health:IsDead() then
                    v.components.health:DoDelta(math.asin(1*.2)*-51, nil, "buff_medal_redmark")
					v:AddDebuff("buff_medal_redmark","buff_medal_redmark",{noredattack=true})--添加红晶标记
                end
            end
		end
		SpawnMedalFX("willow_throw_flame",target)
		target.components.health:DoDelta(math.asin(marks*.2)*-51, nil, "buff_medal_redmark")
	end
...
--添加红晶标记
local function AddRedMark(target, noredattack)
	if target ~= nil and target:IsValid() and target.components.health ~= nil and not target.components.health:IsDead() then
...
buff_defs.buff_medal_redmark={
	...
	onattachedfn=function(inst, target, followsymbol, followoffset, data)
		AddRedMark(target, data and data.noredattack)
	end,
	onextendedfn=function(inst, target, followsymbol, followoffset, data)
		AddRedMark(target, data and data.noredattack)
	end,
...
----------------------------------------蓝晶标记-------------------------------------------------
--添加蓝晶标记
local function AddBlueMark(inst, target, marknum)
...
buff_defs.buff_medal_bluemark={
	...
	onattachedfn=function(inst, target, followsymbol, followoffset, data)
		AddBlueMark(inst, target, data and data.marknum)
	end,
	onextendedfn=function(inst, target, followsymbol, followoffset, data)
		AddBlueMark(inst, target, data and data.marknum)
	end,
