	...
	medal_loss_treasure_map={--遗失藏宝图
		...
		spawnTreasure=function(inst,resonator)--生成宝藏
			if resonator then
				...
				local seedNum = math.floor(gift_value/4)--种子数量
				...
				if seedNum>0 then
					...
					if seed then
						...
						if resonator.components.lootdropper then
							resonator.components.lootdropper:FlingItem(seed,pt)
						else
							seed.Transform:SetPosition(pt.x,pt.y,pt.z)
						end
					end
				end
				...
				if fruitNum>0 then
					...
					if fruit then
						...
						if resonator.components.lootdropper then
							resonator.components.lootdropper:FlingItem(fruit,pt)
						else
							fruit.Transform:SetPosition(pt.x,pt.y,pt.z)
						end
					end
				end
			end
