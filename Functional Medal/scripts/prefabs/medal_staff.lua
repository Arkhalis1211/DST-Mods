...
local function isInsideWall(item)
	if not baits_list[item.prefab] then
		return false
	end
...
local function pickup_func(inst,target,pos,doer,single)
	...
	-- local findList = pick_must_tags--必须标签组
	...
	if target then
		...
		if left then
			...
			if target.prefab == "immortal_fruit_oversized" then
				newitem_code = GetMedalRandomItem(TUNING_MEDAL.IMMORTAL_FRUIT_VARIATION_ROOT,target)--冥冥之中已有定数
				consume_value = TUNING_MEDAL.MEDAL_MOONLIGHT_STAFF_ADDUSE
			--各种巨型作物
			elseif random_change_loot[target.prefab] then
				newitem_code=weighted_random_choice(random_change_loot[target.prefab])
				consume_value = TUNING_MEDAL.MEDAL_MOONLIGHT_STAFF_ADDUSE
		...
		if inst.components.finiteuses and inst.components.finiteuses:GetUses() >= consume_value then
