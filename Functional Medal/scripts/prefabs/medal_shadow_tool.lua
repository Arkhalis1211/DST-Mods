...
local TOOL_DATA = {
	{--斧头
		...
		-- effectiveness=1,--效率
		-- actiontype=1,--动作类型,1工具(默认)、2固定动作、3单纯清除(暂时用不到)
...
local function ResetToolAction(inst)--,newaction,act_type)
	...
	-- local tool_info = TOOL_DATA[inst.tool_idx] or {}
	local act_type = tool_info.actiontype or 1--1工具、2固定动作、3单纯清除
	...
	if act_type==2 then
		inst:AddInherentAction(newaction)
	end
	if inst.components.tool then
...
local function ResetToolAnim(inst, idx)
	idx = idx or (inst.tool_idx == 8 and 1 or inst.tool_idx)
...
local function SorbToolFn(inst,tool,owner)
	...
	if idx == nil or inst.sorb_record[idx] == nil then return false,"UNSUPPORTED" end
	...
	if inst.tool_idx == idx or inst.tool_idx == 8 then--吸收当前对应的工具或五合一工具,则需要重置下效果
		ResetToolAnim(inst)
		ResetToolAction(inst)
	end
