...
local function SetOmmateumVision(inst)
	if inst.components.playervision then
		if inst.medalnightvision and inst.medalnightvision:value() then
			...
			inst.components.playervision:PushForcedNightVision("ommateum_certificate", 1, OMMATEUM_COLOURCUBES, true)
		else
...
AddPlayerPostInit(function(inst)
	...
	inst.CanSoulhop = function(inst, souls)
		...
		if inst:HasTag("medal_map_blinker") and inst.replica.inventory:Has("wortox_soul", souls or 1) then
			...
			if data and data.food and data.feeder then
				...
				elseif data.food.prefab == "shroomcake" or basename == "shroomcake" then
					if data.feeder.medal_confused_mark and data.feeder.medal_confused_mark>0 then
						data.feeder.medal_confused_mark = data.feeder.medal_confused_mark-2--减少2层迷糊标记
						if data.feeder.medal_confused_mark<=0 then
...
local function CheckSoulsAdded(inst)
    ...
    local souls = inst.components.inventory:FindItems(IsSoul)--获取身上的灵魂
	local medal=inst.components.inventory:EquipMedalWithgroupTag("devoursoulMedal")--获取玩家的噬灵系列勋章
    local count = 0
    for i, v in ipairs(souls) do
        count = count + GetStackSize(v)--统计灵魂数量
    end
    if count > TUNING.WORTOX_MAX_SOULS then--如果身上的灵魂总数超过最大携带上限了，就要进行处理了
    	count = count - TUNING.WORTOX_MAX_SOULS--算一下超出了多少个
		local neednum = 0--恢复勋章耐久所需数量
		if medal and medal.components.finiteuses then
			neednum = medal.components.finiteuses.total - medal.components.finiteuses:GetUses()
			neednum = math.min(neednum,count)
			count = count - neednum
		end
    	...
    	for i, v in ipairs(souls) do
			...
			if neednum>0 then--优先给勋章加耐久
				if vcount <= neednum then
					v:Remove()
					medal.components.finiteuses:Repair(vcount)--加耐久
					neednum = neednum - vcount
					vcount = 0
				else
					v.components.stackable:Get(neednum):Remove()
					medal.components.finiteuses:Repair(neednum)--加耐久
					vcount = vcount - neednum
					neednum = 0
				end
			end
			--补完耐久还有剩
			if vcount > 0 and count>0 then
				if vcount <= count then--掉落多余的灵魂
			...
			inst.components.projectile:SetOnHitFn(function(inst, attacker, target)
				if target~=nil and target.components.inventory ~= nil then
					...
					if target.medal_soulstealer~=nil then
						...
						local count = 0--灵魂数量
						--灵魂上限
						local maxsoulsnum = target.prefab=="wortox" and TUNING.WORTOX_MAX_SOULS or 10*target.medal_soulstealer
						for i, v in ipairs(souls) do
...
AddPrefabPostInit("merm",function(inst)
	...
	if oldresolvechatterfn ~= nil then
		inst.components.talker.resolvechatterfn = function(inst, strid, strtbl)
			if ThePlayer and ThePlayer:HasTag("medal_mermfluent") then
...
AddPrefabPostInit("glommerflower",function(inst)
	...
	if TheWorld.ismastersim then
		...
		inst.components.leader.onremovefollower = function(inst, child)
			if oldonremovefollower ~= nil then
				oldonremovefollower(inst, child)
			end
	...
	if TheWorld.ismastersim then
		...
		inst.components.repairable.onrepaired = function(inst, doer, repair_item)
			if oldonrepaired ~= nil then
				oldonrepaired(inst, doer, repair_item)
			end
...
AddClassPostConstruct("components/inventoryitem_replica", function(self)
    ...
    self.GetDeployPlacerName = function(self)
		...
		if oldGetDeployPlacerName then
...
AddComponentPostInit("stewer", function(self)
	...
	self.Harvest = function(self,harvester)
		if self.done and harvester ~= nil and self.chef_id == harvester.userid and self.product then
			...
			--红晶锅也能正常解锁食谱书
			if self.inst.prefab=="medal_cookpot" then
...
AddComponentPostInit("playercontroller", function(self)
	...
	self.GetAttackTarget = function(self, force_attack, force_target, isretarget, use_remote_predict)
		...
		if oldGetAttackTarget then
			return oldGetAttackTarget(self, force_attack, force_target, isretarget, use_remote_predict)
		end
	end
end)
...
AddComponentPostInit("combat", function(self)
	...
	self.DoAreaAttack = function(self, target, range, weapon, validfn, stimuli, excludetags)
...
AddComponentPostInit("spellbook", function(self)
	...
	self.CanBeUsedBy=function(self, action)
		...
		if oldCanBeUsedBy then
			return oldCanBeUsedBy(self,action)
		end
	end
end)
...
AddComponentPostInit("planarentity", function(self)
	...
	self.AbsorbDamage=function(self, damage, attacker, weapon, spdmg)
		if self.inst:HasTag("chaos_creature") then
			...
			local death_times = TheWorld and TheWorld.components.medal_infosave and TheWorld.components.medal_infosave:GetChaosCreatureDeathTimes(self.inst) or 0
