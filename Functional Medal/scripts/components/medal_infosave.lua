...
--混沌生物key映射表
local chaos_creature_map = {
	medal_beeguard = "medal_beequeen",--堕落之蜂用凋零之蜂的次数
}
...
function medal_infosave:GetChaosCreatureDeathTimes(target)
	if target ~= nil and target.prefab then
		local newkey = chaos_creature_map[target.prefab] or target.prefab
