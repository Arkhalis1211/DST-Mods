...
local function SetLeader(inst)
	local leader = inst.components.follower.leader
	if leader ~= nil then
		inst:onLeaderChanged(leader)
	--	print("onLeaderChanged from SetLeader")
	end
end
...
function LavaeWflyBrain:OnStart()
    ...
    PriorityNode({
		...
		DoAction(self.inst, SetLeader), -- Check leader for skill tree changes
